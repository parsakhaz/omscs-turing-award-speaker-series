...
const SidebarGallery: React.FC<SidebarGalleryProps> = ({
  currentSpeaker,
  speakers,
  advisors,
}) => {
  ...
  const allItems = [
    ...speakers.map(s => ({ ...s, type: 'speaker' })),
    ...advisors.map(a => ({ ...a, type: 'advisor' })),
  ].filter(item => item.name !== currentSpeaker);
        ...
        <Link href={`/${item.type}/${item.slug}`} key={item.slug}>
          <div className="flex items-center space-x-3 p-2 rounded-lg hover:bg-gray-100 transition-colors duration-300">
            ...
            <div>
              ...
              {item.distinction && (
                <p className="text-xs text-gray-600">{item.distinction}</p>
              )}
            </div>
          </div>
        </Link>
    ...
    <>
      ...
      <div className="fixed bottom-4 right-4 z-50">
        <button
          ...
          className="bg-[#013057] text-white p-4 rounded-full shadow-lg flex items-center justify-center"
        >
