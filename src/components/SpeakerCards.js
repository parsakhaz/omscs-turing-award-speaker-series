...
const SpeakerBio = ({ name, dateTime, description, rsvpLink, turingAwardWinner }) => {
	if (turingAwardWinner) {
		return (
			<div className='card drop-shadow-md rounded-sm bg-base-100 shadow-xl flex flex-col outline outline-[#a4925a]'>
				...
				<a target='_blank' rel='noreferrer' href={rsvpLink} className=' bg-[#013057] mt-4 text-white hover:text-slate-500 p-4 text-center text-[0.8rem]'>
					RSVP to {name}&apos;s Talk
				</a>
			</div>
		);
